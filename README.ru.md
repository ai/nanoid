...
* **Compact.** Алфавит в ID больше, чем у UUID (`A-Za-z0-9_-`).
...
1. Nano ID использует более широкий алфавит и сравнимое количество
   бит случайности будут упакованы в строку в 21 символа, против 36 у UUID.
...
Вы так же можете [сменить алфавит ID](#смена-алфавита-или-длины) или
...
1. Прочитайте документация [`react-native-get-random-values`] и установить его.
...
Для Rollup вам нужен плагины [`@rollup/plugin-node-resolve`]
...
В PouchDB и CouchDB, IDs не могут начинаться на `_`. Добавьте к ID префикс,
...
Веб-воркер не имеют доступа к аппаратному генератору случайности.
...
Если вам не нужны непредсказуемость ID, то в Веб-воркере можно использовать
...
Так же можно заменить алфавит у небезопасной и асинхронной версиях.
