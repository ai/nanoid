...
Из-за особенной работы генератора случайных чисел может быть медленнее
других модулей, но чаще всего эта разница будет незначительна.
...
Но если использовать асинхронный API у Nano ID, то процесс будет
...
Но если вам не нужна криптографическая стойкость, то вы можете ускорить генерацию ID,
...
Но учтите, что предсказуемость ID может быть использована для взлома системы.
